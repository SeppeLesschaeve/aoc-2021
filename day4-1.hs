...
getWinningOutCome :: [Int] -> [[[(Int,Bool)]]] -> (Int, [[(Int, Bool)]])
getWinningOutCome (n:numbers) cards
  | winCard == [] = getWinningOutCome numbers newCards
  | otherwise     = (n, winCard) 
  where 
    newCards = markNumberCards n cards
    winCard  = getWinningCard newCards

getWinning :: ([Int], [[[(Int,Bool)]]]) -> (Int, [[(Int, Bool)]])
getWinning (numbers, cards) = getWinningOutCome numbers cards
