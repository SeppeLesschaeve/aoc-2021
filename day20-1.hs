...
solve parsed = length . M.toList . M.filter (== '#') $ steps 2 (snd parsed) (fst parsed) False
...
render img = unlines $ map (map snd) $ groupBy (\a b -> fst (fst a) == fst (fst b)) $ sortBy (comparing fst) $ M.toList img
