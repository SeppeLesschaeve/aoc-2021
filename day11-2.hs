...
isSyncRow :: [Int] -> Bool
isSyncRow [] = True
isSyncRow (cell:cells) = (cell == 0) && (isSyncRow cells)

isSyncMatrix :: [[Int]] -> Bool
isSyncMatrix [] = True
isSyncMatrix (row:rows) = (isSyncRow row) && (isSyncMatrix rows)
    
trackFlashesSync :: Int -> [[Int]] -> Int
trackFlashesSync n flashMatrix
  | isSyncMatrix subFlashMatrix = n
  | otherwise             = trackFlashesSync (n+1) subFlashMatrix 
  where 
...
main = do
  ...
  return (trackFlashesSync 1 (getMatrix content))
