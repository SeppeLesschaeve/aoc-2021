...
incrementMatrix :: [[Int]] -> Int -> [[Int]]
incrementMatrix [] _ = []
incrementMatrix (row:rows) amount = (map (+amount) row):(incrementMatrix rows amount)

enlargeRowHorizon :: [Int] -> [Int] -> Int -> Int -> [Int]
enlargeRowHorizon row originalRow curAmount amount
  | curAmount == amount = row 
  | otherwise = enlargeRowHorizon (row ++ newRow) originalRow (curAmount+1) amount
  where newRow = map (+curAmount) originalRow
  
enlargeHorizon :: [[Int]] -> Int -> [[Int]]
enlargeHorizon [] _ = []
enlargeHorizon (row:rows) amount = (enlargeRowHorizon row row 1 amount):(enlargeHorizon rows amount) 

enlargeVertic :: [[Int]] -> [[Int]] -> Int -> Int -> [[Int]]
enlargeVertic matrix originalMatrix curAmount amount
  | curAmount == amount = matrix
  | otherwise = enlargeVertic (matrix ++ newMatrix) originalMatrix (curAmount+1) amount
  where newMatrix = incrementMatrix originalMatrix curAmount

enlargedMatrix :: [[Int]] -> Int -> [[Int]] 
enlargedMatrix risks amount = enlargeVertic enlargedHorizon enlargedHorizon 1 amount  
  where 
    enlargedHorizon = enlargeHorizon risks amount  
...
main = do
  ...
  return (findLstRisk (enlargedMatrix (getMatrix content) 5)) 
